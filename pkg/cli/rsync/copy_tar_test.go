	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			options := &RsyncOptions{
				...
				RsyncLast:    3, // Enable file discovery
			...
			// Make sure --last is in the ignored flags.
			actualIgnored := sets.New(strategy.IgnoredFlags...)
			if !actualIgnored.Has("--last") {
				t.Errorf("--last should be in ignored flags")
			}
		})
	}
}
